...
const root = commandpost
    .create<
        ...
        {}
    >("emailchecker [options]")
    .version(require("../package.json").version, "-v, --version")
    .description("Scans for validity of a single or multiple email addresses.")
    .option(
        "-i, --stdin",
        "If provided, reads email input from stdin. One email per line",
    )
    ...
    .action(async (opts: any) => {
        ...
        if (opts.stdin) {
            ...
            await checkParallel(emailsToCheck, 10).then((results: any) =>
        ...
        if (opts.mongoUrl) {
            ...
            mongoose
                .connect(opts.mongoUrl[0], { useNewUrlParser: true })
                .then(() => {
                    log.debug(`Connection ${opts.mongoUrl} opened`)
                    return new Promise((resolve: any) => {
                        const cursor = BadooModel.find(
                        ...
                        cursor.on("end", async () => {
                            log.info("End:")
                            await new Promise((doneWait: any) =>
                            ...
                            until(
                                ...
                                (callback: any) => {
                                    ...
                                    _.range(opts.threads).forEach(() =>
                                        partEmails.push(emails.pop()),
                                    )
                                    log.debug(`Oustanding: ${emails.length}`)
                                    log.info(JSON.stringify(partEmails))
                                    checkParallel(partEmails, opts.threads)
                                        .then((results: any) => {
