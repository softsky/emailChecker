...
const checkParallel = (
    ...
    new Promise((resolve) => {
        const asyncQueue = emails.map((email: string) => {
            ...
            // };
            return (callback: any) =>
                job(email).then((it: any) => callback(null, it))
        })
        parallelLimit(asyncQueue, numSeries, (err: any, results: any) => {
            cb(err, results)
            if (err) {
                log.error(err)
            }
...
const checkForDomain = (o: object, opts: any = {}) =>
            ...
            dns.resolveMx(domain, (err, addresses) => {
                if (err) {
                   ...
                   ascending order, and then contact the first exchange. */
                    ...
                    log.debug(
                        `Using exchanges: ${exchange} of ${JSON.stringify(
                            addresses,
                        )}`,
                    )
                   ...
                   built-in net client. */
                    ...
                    conn.on("connect", async () => {
                        const EOL = "\r\n"
                        let conversation: any[] = ["QUIT"]
                        const results: any[] = []

                        conversation = conversation.concat(
                            locals.map(
                                (local: string) => `RCPT TO: ${local}@${domain}`,
                            ),
                        )
                       ...
                       malware from discovering all available addresses. */
                        ...
                        conn.on("data", (data: any) => {
                            ...
                            if (
                                ...
                                results[
                                    results.length - 1
                                ].status = data.startsWith("250 ") ? true : data
                            }
